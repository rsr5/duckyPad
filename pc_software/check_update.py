...
firmware_url = (
    "https://api.github.com/repos/dekuNukem/duckyPad/contents/firmware?ref=master"
)
...
def get_firmware_update_status(current_version):
    try:
        ...
        dfu_list = [
            x["name"]
            for x in file_list
            if "name" in x and "type" in x and x["type"] == "file"
        ]
        dfu_list = [
            d.replace("duckypad_v", "").replace(".dfu", "")
            for d in dfu_list
            if d.startswith("duckypad_v") and d.endswith(".dfu")
        ]

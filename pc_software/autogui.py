...
def parse_line(ducky_line):
    ...
    if len(ducky_line) <= 0:
        ...
        parse_result, parse_note = parse_combo(
            ducky_line[len(cmd_KEYDOWN) :], ACTION_PRESS_ONLY
        )
    elif ducky_line.startswith(cmd_KEYUP):
        parse_result, parse_note = parse_combo(
            ducky_line[len(cmd_KEYUP) :], ACTION_RELEASE_ONLY
        )
    elif ducky_line.startswith(cmd_STRING):
        pyautogui.write(
            ducky_line[len(cmd_STRING) :], interval=default_char_delay_ms / 1000
        )
...
def execute_file(str_list):
    ...
    for index, line in enumerate(str_list):
        try:
            ...
            return PARSE_ERROR, "parse error on line " + str(
                index + 1
            ) + ":\n\n" + line + "\n\n" + str(e)

        ...
        try:
            with open(os.path.join(os.path.dirname(path), "config.txt")) as ffffff:
                for line in ffffff:
                    ...
                    if (
                        this_split[0].startswith("z")
                        and int(this_split[0][1:]) == index
                    ):
                        ret = this_split[1]
                        break
        except Exception as e:
...
class dp_profile(object):
    def read_keys(self, path):
        key_file_list = [
            ...
            if x.endswith(".txt") and x.startswith("key") and x[3].isnumeric()
        ]
        key_file_list.sort(
            key=lambda s: int(s[3:].split("_")[0].split(".txt")[0])
        )  # sort by number not by letter
        # print(key_file_list)
    ...
    def load_from_path(self, path):
        ...
        if not (
            folder_name.startswith("profile")
            and folder_name[7].isnumeric()
            and "_" in folder_name
        ):
    ...
    def __str__(self):
        ...
        ret += (
            "key count:\t" + str(len([x for x in self.keylist if x is not None])) + "\n"
        )
        ret += "keys:\n"
...
def build_profile(root_dir_path):
    my_dirs = [
        ...
        if os.path.isdir(os.path.join(root_dir_path, d))
    ]
    my_dirs = [
        x for x in my_dirs if x.startswith("profile") and x[7].isnumeric() and "_" in x
    ]
    my_dirs.sort(key=lambda s: int(s[7:].split("_")[0]))
    my_dirs = [
        os.path.join(root_dir_path, d) for d in my_dirs if d.startswith("profile")
    ]
...
def load_keymap_from_file(file_path):
    ...
    this_keymap.display_name = this_keymap.file_name.replace("dpkm_", "").replace(
        ".txt", ""
    )
...
def load_keymap(root_dir_path):
    ...
    keymap_file_list = [
        d
        for d in os.listdir(keymap_folder_path)
        if d.startswith("dpkm_") and d.endswith(".txt")
    ]
    return_list = []
...
def load_online_keymap():
    ...
    for item in file_list:
        ...
        this_keymap.url = item[
            "download_url"
        ]  # download the content later, when saving
        this_keymap.is_valid = 1
        return_list.append(this_keymap)
    return return_list

...
LOGGING_CONFIG = {
    ...
    "loggers": {
        ...
        "hid_op": {"handlers": ["default"], "level": LOGGING_LEVEL, "propagate": False},
    },
}
...
def get_duckypad_path():
    if "win32" in sys.platform:
        for device_dict in hid.enumerate():
            if (
                device_dict["vendor_id"] == 0x0483
                and device_dict["product_id"] == 0xD11C
                and device_dict["usage"] == 58
            ):
...
def dump_from_hid(save_path, string_var):
    ...
    for item in file_struct_list:
        ...
        if item.type == 1:
            ...
            for fff in files_in_this_dir:
                ...
                lv2_list.append(
                    my_file_obj(
                        fff[0], fff[1], duckypad_read_file(item.name + "/" + fff[0])
                    )
                )
    ...
    for item in file_struct_list:
        if item.type == 0 and item.content is not None:
            with open(
                os.path.join(save_path, item.name), "w", encoding="latin-1"
            ) as this_file:
        ...
        if item.type == 1 and item.content is not None:
            ...
            for subfile in item.content:
                if subfile.type == 0 and subfile.content is not None:
                    with open(
                        os.path.join(this_folder_path, subfile.name),
                        "w",
                        encoding="latin-1",
                    ) as this_file:
...
def duckypad_hid_file_sync(duckypad_dir_name, local_dir_name, string_var):
    compare_result = filecmp.dircmp(
        duckypad_dir_name, local_dir_name, ignore=["keymaps", "SYSTEM~1"]
    )
    ...
    for item in top_level_to_copy:
        ...
        sub_dir_path = [
            (os.path.join(item_path, x), os.path.join(item, x))
    ...
    for common_dir_name in compare_result.common_dirs:
        subdir_compare_result = filecmp.dircmp(
            os.path.join(duckypad_dir_name, common_dir_name),
            os.path.join(local_dir_name, common_dir_name),
        )

        subdir_file_to_copy = (
            subdir_compare_result.right_only + subdir_compare_result.diff_files
        )

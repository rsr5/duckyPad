...
def open_discord_link():
    try:
        webbrowser.open(
            str(
                urllib.request.urlopen(discord_link_url).read().decode("latin-1")
            ).split("\n")[0]
        )
    except Exception as e:
...
def create_help_window():
    ...
    user_manual_label = Label(
        master=help_window, text="Not sure what to do? Please read..."
    )
    user_manual_label.place(x=35, y=5)
    user_manual_button = Button(
        help_window, text="User Manual", command=open_duckypad_user_manual_url
    )
    ...
    troubleshoot_button = Button(
        help_window,
        text="Troubleshooting Guides",
        command=open_duckypad_troubleshooting_url,
    )
    ...
    discord_button = Button(
        help_window, text="Official Discord Chatroom", command=open_discord_link
    )
...
def open_duckypad_troubleshooting_url():
    webbrowser.open(
        "https://github.com/dekuNukem/duckyPad/blob/master/troubleshooting.md"
    )
...
def print_fw_update_label():
    ...
    if fw_result == 0:
        dp_fw_update_label.config(
            text="Firmware (" + str(this_version) + "): Up to date",
            fg="black",
        ...
        dp_fw_update_label.config(
            text="Firmware (" + str(this_version) + "): Update available! Click me!",
            fg="black",
            bg="orange",
            cursor="hand2",
        )
        ...
        dp_fw_update_label.config(
            text="Firmware: Unknown", fg="black", bg=default_button_color
        )
        dp_fw_update_label.unbind("<Button-1>")
...
def connect_button_click():
    ...
    if hid_op.get_duckypad_path() is None:
        if (
            messagebox.askokcancel(
                "Info", "duckyPad not found!\n\nConfigure via SD card instead?"
            )
            == False
        ):
            return
    ...
    if init_success is False and "linux" in sys.platform:
        box_result = messagebox.askyesnocancel(
            "Info",
            "duckyPad detected, but you need to add an udev rule before I can access it.\n\nClick Yes to see instructions\n\nClick No to configure via SD card.",
        )
    ...
    if init_success is False and "darwin" in sys.platform and is_root() is False:
        box_result = messagebox.askyesnocancel(
            "Info",
            "duckyPad detected, but this app lacks permission to access it.\n\nClick Yes to see instructions\n\nClick No to configure via SD card.",
        )
        if box_result is True:
            webbrowser.open(
                "https://github.com/dekuNukem/duckyPad/blob/master/troubleshooting.md#autoswitcher--usb-configuration-isnt-working-on-macos"
            )
        elif box_result is False:
            select_root_folder()
        return
    ...
    if init_success is False and "darwin" in sys.platform and is_root() is True:
        box_result = messagebox.askyesnocancel(
            "Info",
            "duckyPad detected, however, due to macOS restrictions, you'll need to enable some privacy settings.\n\nClick Yes to learn how.\n\nClick No to configure via SD card.",
        )
        if box_result is True:
            webbrowser.open(
                "https://github.com/dekuNukem/duckyPad/blob/master/troubleshooting.md#autoswitcher--usb-configuration-isnt-working-on-macos"
...
def profile_shift_up():
    ...
    profile_list[destination], profile_list[source] = (
        profile_list[source],
        profile_list[destination],
    )
    update_profile_display()
    ...
    for count, item in enumerate(profile_list[profile_index].keylist):
        if item is not None:
            ...
            if item.color is not None:
                ...
                key_button_list[count].config(
                    background=rgb_to_hex(profile_list[profile_index].bg_color)
                )
                this_color = profile_list[profile_index].bg_color
            key_button_list[count].config(
                text=item.name[:7], foreground=adapt_color(this_color)
            )
        elif item is None and profile_list[profile_index].dim_unused is False:
            key_button_list[count].config(
                text="", background=rgb_to_hex(profile_list[profile_index].bg_color)
            )
...
def clean_input(str_input, len_limit=None, clean_filename=True):
    result = "".join(
        [
            x
            for x in str_input
            if 32 <= ord(x) <= 126 and x not in invalid_filename_characters
        ]
    )
...
def profile_dupe_click():
    ...
    answer = simpledialog.askstring(
        ...
        "New name?",
        parent=profiles_lf,
        initialvalue=profile_list[selection[0]].name,
    )
    if answer is None:
        return
    answer = clean_input(answer, 13)
...
def validate_data_objs(save_path):
    ...
    for profile_index, this_profile in enumerate(profile_list):
        this_profile.path = os.path.join(
            save_path, "profile" + str(profile_index + 1) + "_" + str(this_profile.name)
        )
        for key_index, this_key in enumerate(this_profile.keylist):
            if this_key is None:
                continue
            this_key.path = os.path.join(
                this_profile.path, "key" + str(key_index + 1) + ".txt"
            )
...
def dump_keymap(save_path):
    global sd_card_keymap_list
    file_list = [
        d for d in os.listdir(save_path) if d.startswith("dpkm_") and d.endswith(".txt")
    ]
...
def save_everything(save_path):
    ...
    try:
        ...
        my_dirs = [
            x
            for x in my_dirs
            if x.startswith("profile") and x[7].isnumeric() and "_" in x
        ]
        my_dirs = [
            os.path.join(save_path, d) for d in my_dirs if d.startswith("profile")
        ]
        for item in my_dirs:
            try:
                shutil.rmtree(item)
        ...
        for this_profile in profile_list:
            ...
            for this_key in this_profile.keylist:
                ...
                this_key.check_loop()
                config_file.write(
                    "z" + str(this_key.index) + " " + str(this_key.name) + "\n"
                )
        ...
        try:
            ...
            for index, line in enumerate(dp_settings.list_of_lines):
                if "sleep_after_min" in line:
                    found = True
                    dp_settings.list_of_lines[index] = (
                        "sleep_after_min " + str(sleepmode_slider.get()) + "\n"
                    )
            if found is False:
                dp_settings.list_of_lines.append(
                    "sleep_after_min " + str(sleepmode_slider.get()) + "\n"
                )
        except Exception as e:
            print("dps", e)
...
def key_button_click(button_widget):
    ...
    if profile_list[profile_index].keylist[selected_key] is not None:
        key_name_entrybox.insert(
            0, profile_list[profile_index].keylist[selected_key].name
        )
        script_textbox.config(state=NORMAL)
        script_textbox.delete(1.0, "end")
        script_textbox.insert(
            1.0, profile_list[profile_index].keylist[selected_key].script.rstrip("\n")
        )
    ...
    if profile_list[profile_index].keylist[selected_key].color is None:
        ...
        key_color_button.config(
            background=rgb_to_hex(
                profile_list[profile_index].keylist[selected_key].color
            )
        )
...
root_folder_select_button = Button(
    root_folder_lf, text="Connect", command=connect_button_click
)
root_folder_select_button.place(x=75, y=0, width=75)

root_folder_path_label = Label(
    master=root_folder_lf, textvariable=dp_root_folder_display, foreground="red"
)
...
save_as_button = Button(
    root_folder_lf, text="Backup...", command=backup_button_click, state=DISABLED
)
...
profiles_lf = LabelFrame(
    root,
    text="Profiles",
    width=int(MAIN_WINDOW_WIDTH / 3 - PADDING * 1.3),
    height=MAIN_COLOUM_HEIGHT - HIGHT_ROOT_FOLDER_LF - PADDING,
)
profiles_lf.place(x=PADDING, y=HIGHT_ROOT_FOLDER_LF)
root.update()

profile_lstbox = Listbox(
    profiles_lf, listvariable=profile_var, height=16, exportselection=0
)  # , selectmode='single'?
profile_lstbox.place(x=32, y=PADDING, width=182, height=270)
profile_lstbox.bind("<<ListboxSelect>>", on_profile_lstbox_select)

profile_up_button = Button(
    profiles_lf, text="↑", command=profile_shift_up, state=DISABLED
)
profile_up_button.place(x=5, y=80, width=20, height=40)

profile_down_button = Button(
    profiles_lf, text="↓", command=profile_shift_down, state=DISABLED
)
...
profile_add_button = Button(
    profiles_lf, text="New", command=profile_add_click, state=DISABLED
)
profile_add_button.place(
    x=PADDING * 2, y=BUTTON_Y_POS, width=BUTTON_WIDTH, height=BUTTON_HEIGHT
)

profile_dupe_button = Button(
    profiles_lf, text="Duplicate", command=profile_dupe_click, state=DISABLED
)
profile_dupe_button.place(
    x=PADDING * 2.5 + BUTTON_WIDTH,
    y=BUTTON_Y_POS,
    width=BUTTON_WIDTH,
    height=BUTTON_HEIGHT,
)

profile_remove_button = Button(
    profiles_lf, text="Remove", command=profile_remove_click, state=DISABLED
)
profile_remove_button.place(
    x=PADDING * 2,
    y=BUTTON_Y_POS + BUTTON_HEIGHT + int(PADDING / 2),
    width=BUTTON_WIDTH,
    height=BUTTON_HEIGHT,
)

profile_rename_button = Button(
    profiles_lf, text="Rename", command=profile_rename_click, state=DISABLED
)
profile_rename_button.place(
    x=PADDING * 2.5 + BUTTON_WIDTH,
    y=BUTTON_Y_POS + BUTTON_HEIGHT + int(PADDING / 2),
    width=BUTTON_WIDTH,
    height=BUTTON_HEIGHT,
...
dim_unused_keys_checkbox = Checkbutton(
    profiles_lf,
    text="Dim unused keys",
    variable=dim_unused_keys_checkbox_var,
    command=dim_unused_keys_click,
    state=DISABLED,
)
...
kd_R1 = Radiobutton(
    profiles_lf,
    text="      Auto",
    variable=kd_color_var,
    value=0,
    command=kd_radiobutton_auto_click,
    state=DISABLED,
)
...
kd_R2 = Radiobutton(
    ...
    variable=kd_color_var,
    value=1,
    command=kd_radiobutton_custom_click,
    state=DISABLED,
)
...
keys_lf.place(
    x=profiles_lf.winfo_x() + profiles_lf.winfo_width() + PADDING,
    y=profiles_lf.winfo_y(),
)
root.update()
...
def button_drag_start(event):
    ...
    if (
        drag_source_button_index is not None
        and drag_destination_button_index is not None
        and drag_destination_button_index != drag_source_button_index
    ):
        key_button_list[drag_destination_button_index].config(
...
def button_drag_release(event):
    ...
    if (
        ...
        (
            profile_list[profile_index].keylist[drag_destination_button_index],
            profile_list[profile_index].keylist[drag_source_button_index],
        ) = (
            profile_list[profile_index].keylist[drag_source_button_index],
            profile_list[profile_index].keylist[drag_destination_button_index],
        )
...
key_button_xy_list = [
    ...
    (
        KEY_BUTTON_GAP * 2 + KEY_BUTTON_WIDTH,
        KEY_BUTTON_HEADROOM + PADDING * 2 + KEY_BUTTON_HEIGHT,
    ),
    (
        KEY_BUTTON_GAP * 3 + KEY_BUTTON_WIDTH * 2,
        KEY_BUTTON_HEADROOM + PADDING * 2 + KEY_BUTTON_HEIGHT,
    ),
    (KEY_BUTTON_GAP, KEY_BUTTON_HEADROOM + PADDING * 3 + KEY_BUTTON_HEIGHT * 2),
    (
        KEY_BUTTON_GAP * 2 + KEY_BUTTON_WIDTH,
        KEY_BUTTON_HEADROOM + PADDING * 3 + KEY_BUTTON_HEIGHT * 2,
    ),
    (
        KEY_BUTTON_GAP * 3 + KEY_BUTTON_WIDTH * 2,
        KEY_BUTTON_HEADROOM + PADDING * 3 + KEY_BUTTON_HEIGHT * 2,
    ...
    (
        KEY_BUTTON_GAP * 2 + KEY_BUTTON_WIDTH,
        KEY_BUTTON_HEADROOM + PADDING * 4 + KEY_BUTTON_HEIGHT * 3,
    ),
    (
        KEY_BUTTON_GAP * 3 + KEY_BUTTON_WIDTH * 2,
        KEY_BUTTON_HEADROOM + PADDING * 4 + KEY_BUTTON_HEIGHT * 3,
    ...
    (
        KEY_BUTTON_GAP * 2 + KEY_BUTTON_WIDTH,
        KEY_BUTTON_HEADROOM + PADDING * 5 + KEY_BUTTON_HEIGHT * 4,
    ),
    (
        KEY_BUTTON_GAP * 3 + KEY_BUTTON_WIDTH * 2,
        KEY_BUTTON_HEADROOM + PADDING * 5 + KEY_BUTTON_HEIGHT * 4,
...
for x in range(15):
    ...
    this_button.place(
        x=key_button_xy_list[x][0],
        y=key_button_xy_list[x][1],
...
key_rename_button = Button(
    keys_lf, text="Apply", command=key_rename_click, state=DISABLED
)
key_rename_button.place(
    x=KEY_BUTTON_GAP, y=340, width=BUTTON_WIDTH, height=BUTTON_HEIGHT
)
root.update()
key_remove_button = Button(
    keys_lf, text="Remove", command=key_remove_click, state=DISABLED
)
key_remove_button.place(
    x=KEY_BUTTON_GAP * 2 + key_rename_button.winfo_width(),
    y=340,
    width=BUTTON_WIDTH,
    height=BUTTON_HEIGHT,
)
...
key_color_rb1 = Radiobutton(
    keys_lf,
    text="Same as background",
    variable=key_color_type_var,
    value=0,
    command=key_color_rb1_click,
    state=DISABLED,
)
key_color_rb1.place(x=85, y=380)
key_color_rb2 = Radiobutton(
    keys_lf,
    text="",
    variable=key_color_type_var,
    value=1,
    command=key_color_rb2_click,
    state=DISABLED,
)
...
scripts_lf.place(
    x=keys_lf.winfo_x() + keys_lf.winfo_width() + PADDING, y=keys_lf.winfo_y()
)
...
script_instruction = Label(
    master=scripts_lf, text="Read more about Duckyscript...", fg="blue", cursor="hand2"
)
root.update()
...
def script_textbox_modified():
    ...
    if profile_list[profile_index].keylist[selected_key] is not None:
        profile_list[profile_index].keylist[selected_key].script = script_textbox.get(
            1.0, END
...
def run_script():
    ...
    try:
        ...
        time.sleep(2)
        exe_result, exe_note = autogui.execute_file(
            script_textbox.get("1.0", END).replace("\r", "").split("\n")
        )
        if exe_result != 0:
...
script_textbox.place(
    x=key_button_list[0].winfo_x(),
    y=KEY_BUTTON_HEADROOM + PADDING - 3,
    width=key_button_list[-1].winfo_x() + KEY_BUTTON_WIDTH - KEY_BUTTON_GAP,
    height=key_button_list[-1].winfo_y()
    - key_button_list[0].winfo_y()
    + KEY_BUTTON_HEIGHT
    + 5,
)
root.update()
...
script_common_commands_lf = LabelFrame(
    scripts_lf, text="Common commands", width=script_textbox.winfo_width(), height=105
)
script_common_commands_lf.place(x=PADDING, y=300)
root.update()

check_syntax_lf = LabelFrame(
    scripts_lf, text="Code check", width=script_textbox.winfo_width() / 2, height=40
)
...
execute_button = Button(
    scripts_lf, text="Run this script!", command=run_script, state=DISABLED
)
execute_button.place(x=135, y=417, width=105, height=BUTTON_HEIGHT)
root.update()

script_button_xy_list = [
    (SCRIPT_BUTTON_GAP, PADDING),
    (SCRIPT_BUTTON_GAP * 2 + SCRIPT_BUTTON_WIDTH, PADDING),
    (SCRIPT_BUTTON_GAP * 3 + SCRIPT_BUTTON_WIDTH * 2, PADDING),
    (SCRIPT_BUTTON_GAP, PADDING + BUTTON_HEIGHT + 2),
    (SCRIPT_BUTTON_GAP * 2 + SCRIPT_BUTTON_WIDTH, PADDING + BUTTON_HEIGHT + 2),
    (SCRIPT_BUTTON_GAP * 3 + SCRIPT_BUTTON_WIDTH * 2, PADDING + BUTTON_HEIGHT + 2),
    (SCRIPT_BUTTON_GAP, (PADDING + BUTTON_HEIGHT) * 2 + 2),
    (SCRIPT_BUTTON_GAP * 2 + SCRIPT_BUTTON_WIDTH, (PADDING + BUTTON_HEIGHT) * 2 + 2),
    (
        SCRIPT_BUTTON_GAP * 3 + SCRIPT_BUTTON_WIDTH * 2,
        (PADDING + BUTTON_HEIGHT) * 2 + 2,
    ),
]
script_button_commands = [
    "STRING",
    "CTRL",
    "SHIFT",
    "ALT",
...
for x in range(9):
    ...
    this_button = Button(
        script_common_commands_lf,
        text=script_button_commands[x],
        command=this_func,
        state=DISABLED,
    )
    this_button.place(
        x=script_button_xy_list[x][0],
        y=script_button_xy_list[x][1],
        width=SCRIPT_BUTTON_WIDTH,
        height=BUTTON_HEIGHT,
    )
...
def check_syntax_click():
    ...
    for count, line in enumerate(
        profile_list[profile_index].keylist[selected_key].script.split("\n")
    ):
        ...
        if ds_syntax_check.parse_line(line) != ds_syntax_check.PARSE_OK:
            ...
            script_textbox.tag_add(
                "error", str(count + 1) + ".0", str(count + 1) + ".0 lineend"
            )
...
def online_keymap_add_to_sd_button_click():
    ...
    if len(sd_card_keymap_list) >= 7:
        return
    if online_keymap_list[index - 1].file_name in [
        x.file_name for x in sd_card_keymap_list
    ]:
        return
...
def add_local_keymap_to_sd_card_button_click():
    ...
    if not (nameonly.startswith("dpkm_") and nameonly.endswith(".txt")):
        override_name_check = messagebox.askokcancel(
            "Warning", "It doesn't look like a valid keymap file. Load it anyway?"
        )
...
def keymap_instruction_click(event):
    webbrowser.open(
        "https://github.com/dekuNukem/duckyPad/blob/master/keymap_instructions.md"
    )
...
def create_keyboard_layout_window():
    ...
    kbl_online_listbox = Listbox(
        kbl_window, listvariable=online_keymap_var, height=8, exportselection=0
    )  # , selectmode='single'?
    ...
    kbl_on_sd_card_listbox = Listbox(
        kbl_window, listvariable=sd_keymap_var, height=8, exportselection=0
    )  # , selectmode='single'?
    kbl_on_sd_card_listbox.place(x=295, y=30, width=160, height=150)

    online_keymap_add_to_sd_button = Button(
        kbl_window, text="Add\n--->", command=online_keymap_add_to_sd_button_click
    )
    online_keymap_add_to_sd_button.place(x=185, y=45, width=100)
    remove_keymap_from_sd_card_button = Button(
        kbl_window, text="Remove", command=remove_keymap_from_sd_card_button_click
    )
    remove_keymap_from_sd_card_button.place(x=185, y=100, width=100)
    add_local_keymap_to_sd_card_button = Button(
        kbl_window,
        text="Add local file",
        command=add_local_keymap_to_sd_card_button_click,
    )
    ...
    keymap_instruction_label = Label(
        master=kbl_window,
        text="Click me to learn more about keymaps, including how to make your own!",
        fg="blue",
        cursor="hand2",
    )
...
sleepmode_slider.config(
    ...
    sliderlength=20,
    orient=HORIZONTAL,
    command=slider_adjust_sleepmode,
)
...
auto_backup_checkbox = Checkbutton(
    settings_lf,
    text="Profile auto-backup",
    variable=auto_backup_checkbox_var,
    command=auto_backup_click,
)
...
if update_stats == 0:
    pc_app_update_label.config(
        text="This app (" + str(THIS_VERSION_NUMBER) + "): Up to date",
        fg="black",
        bg=default_button_color,
    )
    ...
    pc_app_update_label.config(
        text="This app (" + str(THIS_VERSION_NUMBER) + "): Update available! Click me!",
        fg="black",
        bg="orange",
        cursor="hand2",
    )
    ...
    pc_app_update_label.config(
        text="This app (" + str(THIS_VERSION_NUMBER) + "): Unknown",
        fg="black",
        bg=default_button_color,
    )
    pc_app_update_label.unbind("<Button-1>")
...
keyboard_layout_button = Button(
    settings_lf,
    text="Keyboard Layouts...",
    command=create_keyboard_layout_window,
    state=DISABLED,
)
...
def t1_worker():
    ...
    while 1:
        ...
        if current_hid_op == HID_SAVE:
            ...
            try:
                hid_op.duckypad_hid_init()
                hid_op.duckypad_hid_file_sync(
                    hid_dump_path, hid_modified_dir_path, dp_root_folder_display
                )
                ...
                messagebox.showerror(
                    "error", "Save error: " + str(traceback.format_exc())
                )
                dp_root_folder_display.set("Save FAILED!")

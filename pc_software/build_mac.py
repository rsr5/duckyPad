...
new_name = os.path.join(
    output_folder_path,
    "duckypad_config_" + THIS_VERSION + "_macOS_" + str(platform.mac_ver()[0]) + ".app",
)
...
f.write(
    "sudo "
    + str(os.path.join(new_name, "Contents", "MacOS", "duckypad_config")).replace(
        "dist/", ""
    )
)
f.close()
...
zip_file_name = (
    "duckypad_config_" + THIS_VERSION + "_macOS_" + str(platform.mac_ver()[0]) + ".zip"
)
os.system(
    "cd dist; zip -rv "
    + zip_file_name
